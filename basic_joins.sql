...
select s.user_id, round(COALESCE(AVG(c.action = 'confirmed'),0),2) as confirmation_rate from signups s LEFT join confirmations c on c.user_id = s.user_id   group by s.user_id order by confirmation_rate
